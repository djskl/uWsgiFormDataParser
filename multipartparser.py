...
import base64
import sys
from utils import force_text, unescape_entities, parse_boundary_stream, FIELD, FILE,\
    DEFAULT_CHARSET, parse_header

from datastructures import QueryDict, MultiValueDict
from files.uploadhandler import StopFutureHandlers, SkipFile, StopUpload
import cgi
...
class MultiPartParser(object):
    ...
    def parse(self):
        ...
        if self._content_length == 0:
            return QueryDict('', mutable=True), MultiValueDict()
        ...
        counters = [0] * len(handlers)  #每一个upload handler处理的字节数(最后是文件大小)
        ...
        try:
            for item_type, meta_data, field_stream in Parser(stream, self._boundary):
                ...
                # the type of transformation that has been used in order to 
